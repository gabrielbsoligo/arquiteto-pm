...
export const PipelineView: React.FC = () => {
  const { deals, members, moveDeal, updateDeal, reunioes } = useAppStore();
  // reunião de cada deal: pela FK reuniao_id, senão a mais recente do lead — p/ datas no card
  const reuById = useMemo(() => new Map(reunioes.map(r => [r.id, r])), [reunioes]);
  ...
  const reuForDeal = (d: any) => (d.reuniao_id && reuById.get(d.reuniao_id)) || (d.lead_id && reuByLead.get(d.lead_id)) || undefined;
                            ...
                            <Draggable draggableId={deal.id} index={index} key={deal.id}>
                              {(provided, snapshot) => (
                                <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}
                                  ...
                                  {(() => { const chips = dateChips(reuForDeal(deal), deal.updated_at); return chips.length ? (
