...
function buildHeaderMap(headers: string[]): Partial<Record<keyof typeof FIELD_ALIASES, number>> {
  ...
  // 2 passes: 1º EXATO, 2º SUBSTRING nas colunas restantes — evita que "CNPJ EMPRESA"
  // seja engolida pelo campo empresa. Cada coluna vai a 1 campo só.
  ...
  const assign = (pred: (n: string, a: string) => boolean) => {
    norms.forEach((n, i) => {
      ...
      for (const field of fields) {
        ...
        if (field === "empresa" && ehColunaSocio(n)) continue; // coluna de sócio nunca vira empresa
...
export async function parseFile(file: File, batch: string, nicho = "", owner: string | null = null, origem = ""): Promise<ParseResult> {
  ...
  const comDDD = (numero: string, dddCol: number, row: string[]) => {
    ...
    if (num.length >= 10) return numero;
  ...
  for (let r = 1; r < matrix.length; r++) {
    ...
    const lead: ProspLead = {
      ...
      empresa, cnpj: formatarCNPJ(get(row, "cnpj")), socio1, socio2: get(row, "socio2"),
...
function normalizeLead(l: any): ProspLead {
  ...
  return {
    ...
    ...l,
    origem,
    // CNPJ: preserva fielmente o da lista (formata e recupera zeros à esquerda).
